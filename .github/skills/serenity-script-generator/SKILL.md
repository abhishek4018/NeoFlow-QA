...
Do:
- Convert a passing raw script to feature and step files using direct intelligent Screenplay generation.
- Ensure output paths and naming follow project conventions.
- Run Cucumber + Serenity report pipeline when requested.
- Use Screenplay Pattern (`actorCalled`, `actorInTheSpotlight`, `attemptsTo`, `PageElement`, `Ensure`, `By`).

Do not:
- Author or stabilize raw Playwright scripts from scratch.
- Proceed with conversion if raw replay has not passed.
- Generate direct `page.$` step definitions bypassing Serenity Screenplay APIs.
...
Generate files directly from the validated raw script.

Required capability parity with the converter:
1. Parse raw Playwright actions and assertions (`goto`, `click`, `fill`, `selectOption`, `hover`, `waitFor`, `expect`, `check`, `uncheck`, `press`).
2. Build readable, business-centric Gherkin steps preserving user intent.
3. Generate Serenity/JS step definitions adhering to the Screenplay Pattern using `@serenity-js/core`, `@serenity-js/web`, and `@serenity-js/assertions`.
4. Scan existing `step-definitions/**/*.steps.ts` for duplicate expressions and skip or reuse safely.
5. Detect reusable/common patterns and prefer shared behavior in `step-definitions/generic/generic.steps.ts`.
...
8. Apply generic-first step strategy: reuse existing shared steps before creating codegen-specific steps.
9. When a new pattern is reusable across scenarios, create/extend generic steps in `step-definitions/generic/` and use them immediately.

## Screenplay Implementation Standards
- Import actions from `@serenity-js/web`: `Click`, `Enter`, `Navigate`, `PageElement`, `By`, `isVisible`, `isPresent`, `Text`, `Press`, `Hover`.
- Import assertions from `@serenity-js/assertions`: `Ensure`, `equals`, `contains`, `includes`, `isGreaterThan`, etc.
- Import core orchestration from `@serenity-js/core`: `actorCalled`, `actorInTheSpotlight`.
- Prefer `PageElement.located(By.css(...))` or `PageElement.located(By.xpath(...))` for locator definitions.
...
- Step definitions: `step-definitions/codegen/<name>.steps.ts`
- Reuse shared expressions from `step-definitions/generic/generic.steps.ts`.
...
1. Search `step-definitions/generic/generic.steps.ts` for an existing expression that can satisfy the intent with parameters.
...
- Always run the validation command after generating to verify step definitions link properly.

## Completion Criteria
- Feature and step files are generated at expected paths (`features/codegen/<name>.feature` and `step-definitions/codegen/<name>.steps.ts`).
