...
import { By, Enter, ExecuteScript, isVisible, PageElement } from '@serenity-js/web';
import fs from 'fs';
...
const _GenerateQuestionsButton = () => PageElement.located(By.id('btn-generate-questions')).describedAs('Generate Question Set button');
...
Given('the faculty opens the creator dashboard using the saved magic link token', async () => {
    ...
    try {
        if (fs.existsSync('.magic_link_token.tmp')) {
            const savedUrl = fs.readFileSync('.magic_link_token.tmp', 'utf-8').trim();
    ...
    while (Date.now() - startTime < maxWaitMs) {
        try {
            if (fs.existsSync('.shareable_exam_link.tmp')) {
                const savedUrl = fs.readFileSync('.shareable_exam_link.tmp', 'utf-8').trim();
