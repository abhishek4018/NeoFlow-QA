Feature: Pariksha Assessment Engine Pure UI E2E Workflows
  ...
  @pure_ui @e2e @faculty
  Scenario: Faculty creates, approves, and publishes a public assessment via UI DOM
    ...
    And Alex launches the Faculty Creator Dashboard
    And Alex approves all generated questions in the Approval Queue
    ...
    Then Alex should extract the shareable public assessment link

  @pure_ui @e2e @student
  Scenario: Student registers, completes assessment attempt, and views pedagogical results
    Given a candidate navigates to the published shareable exam link
